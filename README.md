...
進入大廳：主持人建立房間，分享連結給所有參與者。

登錄禮物：每人輸入帶來的禮物內容（例如：星巴克馬克杯），系統會隨機分配每人一個代號。
...
遊戲進行：系統將指令牌洗牌後依序顯示，所有人聽從手機指令動作。
...
建立新專案，並開啟 Authentication > Sign-in method > 啟用 匿名 (Anonymous)。

開啟 Firestore Database，並建立資料庫。

複製你的專案設定 (Config)。

打開 src/App.jsx，找到 firebaseConfig 物件並替換為你的數值：
